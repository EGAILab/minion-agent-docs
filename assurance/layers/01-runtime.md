...
RT-F011/RT-F016 probe added three focused lifecycle tests. RT-F017 then added eight coordinator
tests, three synchronous-observer tests, and one canonical harness test that executes all 26 shared
scenarios.
...
| `runtime_coordinator.rs` | KEEP | RT-001..RT-003, RT-008 | Proves absent-provider pending state, provider-driven activation/revocation, multiple dependents, settled failure state, RT-008 reaction before an unrelated first error, loading invalidation without self-join, and settlement of every affected dependent when one cleanup fails. |
| `runtime_observer.rs` | KEEP | RT-001, RT-009, RT-012, RT-013 | Proves observations occur synchronously at real state/effect/scope boundaries and never drive semantics. |
...
| RT-F017 | HIGH | `CONTRACT_ASSURANCE_DEFECT` — RESOLVED | Rust had correct standalone Fiber/service/event/scope primitives but no coordinated runtime owning dependency notifications/reconciliation, and no Runtime canonical runner. | Fixed with `runtime/coordinator.rs`: `RuntimeCore` composes the existing registries/scopes/events/Fibers, returns a pending handle before reconciliation, and reacts to provider appearance/revocation through real service-registration disposal. `tests/support/runtime_scenario.rs` is a thin adapter and `tests/runtime_conformance.rs` executes all 26 shared scenarios against the typed library. Eight focused coordinator tests cover RT-008, multiple dependents, valid failure settlement, loading invalidation, and failure isolation. No shared artifact changed. |
...
| RT-F020 | HIGH | `CONTRACT_ASSURANCE_DEFECT` — RESOLVED | The initial Rust revocation coordinator stopped its affected-dependent traversal when the first dependent's cleanup failed, allowing later dependents to remain `Active` after their provider disappeared. This was a Rust-local RT-008 violation, not a need for pass-wide aggregation semantics. | Fixed in two phases: invalidate all affected dependents first, then settle each; retain and propagate the first failure after all affected dependents are attempted. `revocation_settles_all_affected_dependents_when_one_cleanup_fails` proves both settled states and preserved failure diagnostics. Unrelated-pass aggregation/order remains unspecified and non-canonical. |
...
Reliability review                       [x]  RT-F011/016/017/020 fixed; RT-F012 and RT-F018 remain recorded non-blocking risks
...
The RT-F012 checkpoint exposed and resolved RT-F020: a cleanup failure in one affected dependent
can no longer strand later affected dependents as `Active`. Rust invalidates and settles the whole
affected set and then returns the first failure. This is RT-008 state preservation, not a canonical
