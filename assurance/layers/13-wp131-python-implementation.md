...
## 8. Remediation 2 -- `CE-L13-WP131-02` revision 6 (`G1` on certified `EXEC-008`; `I001` ordering)

The targeted closure review of remediation 1 left both findings open (§11.8 trigger A), which opened
convergence episode `CE-L13-WP131-02`.
- **Characterization and the owner's decision:** revisions 1-4, then the owner's `G1` (`minion-agent#48` comment `5822609576`).
- **The Layer-12 extension:** `WP-12.E2` / `EXEC-008` `check_readable`, certified in both languages (`minion-agent#62`, `CERTIFIED_CLOSED`).
- **The checkpoint:** revisions 5 and 6. Revision 6 was agreed by Codex's §11.8.5 review (`minion-agent-docs#162` comment `5828260964`, `AGREED_FOR_IMPLEMENTATION` @ `8dff4138`).

This section implements exactly that checkpoint. Remediation 1 above is preserved as history. Its `file_info` access step, code->site rule and `file_info` directory shortcut are superseded.

**`L13-WP131-C012` under `G1`.**
- **Access and content:** `read`'s access stage is `ctx.fs.check_readable` (Pi's `access`, `read.ts:248`), followed by ONE `read_binary_file`; neither receives the signal.
- **Site rule:** the operation that failed owns the site, whatever the code. A `check_readable` failure is `"Cannot access <path>: <phrase>"`; a failure of the content read after it is `"Cannot read <path>: <phrase>"`.
- **Fallback:** `check_readable -> not_supported` (a provider without `EXEC-008`) continues in the disclosed FALLBACK mode, `spec/execution.md` §12.5. There, only a content read's `is_directory`/`not_supported` is the read site.
- **Checkpoint 249:** the `read.ts:249` checkpoint follows `check_readable` in both modes (`CE13-C005`).
- **No other access operations:** `read` uses no `file_info`, `canonical_path` or `EXEC-007` (`CE13-C006`).
- **Checkpoint findings:** `CE13-C001` to `C004` are resolved as revision 5 states.
  - `C001`: an optional capability answered explicitly, with a disclosed fallback.
  - `C002`: H1 at the access site, H2 at the read site.
  - `C003`: an unreadable directory fails the access stage.
  - `C004`: a stable I/O error after a successful access is `Cannot read`.

**`L13-WP131-I001`.**
- **Settle point:** the outcome is decided by order at the work's settle point. `race_abort` wraps the work so that its last synchronous step checks the signal.
- **Abort first:** an abort before settling is `"Operation aborted"`, including over a failure raised after it.
- **Settled first:** a result that settled first stands; it is never re-checked after settling.
- **Unchanged from remediation 1:** the race only decides how soon the rejection is delivered, and in-flight `ctx.fs` work is still never cancelled.
- **Checkpoints:** `read`'s checkpoints now raise `"Operation aborted"` directly; the `None` sentinel is removed.

**Normative deltas (revision 6 inventory, applied in this docs candidate):**
1. **Docs `master`:** `e1d9b817` merged in to carry §12.
2. **`spec/execution.md` §12.5:** one checkpoint in both modes, plus a correction note; §12's status paragraph gains the §12.5 exception. `EXEC-008` semantics and certification are unchanged.
3. **`spec/tools.md`:**
   - TOOL-026 step 5;
   - "`read`: operation mapping" steps 3-4;
   - the site paragraph (R-G1);
   - R010-B's two-site reachable codes;
   - the cancellation paragraph (checkpoint 249 in both modes; I001 ordering);
   - the access-step history (G1 entry);
   - disclosed divergences W-1..W-3 and F-1;
   - the witness matrix.
4. **Manifest `TOOL-025` / `TOOL-039`:** code PR.
5. **Conformance schema:** `provider.check_readable`, `without_exec_008`, `canonical_path`, and a case-level `abort_after`. The runner stays thin (code PR).
6. **Scenarios:** regenerated by `data/13-wp131-python-implementation/gen_scenarios.py`. Every other scenario regenerated byte-identical.

**Witnesses:**
- W-G1..W-G16 are canonical scenarios:
  - `builtin-read-error-text-matches-r010b-closed-vocabulary-per-fserrorcode`;
  - `-provider-without-exec-008-uses-disclosed-fallback`;
  - `-abort-at-the-access-checkpoint-normal-mode` / `-fallback-mode`;
  - `-does-not-depend-on-file-info-or-canonical-path`;
  - `-provider-without-exec-007-reads-normally`.
- W-G6/W-G11/W-G12 on the real host, plus W-I1..W-I4, are in `tests/tools/builtin/test_read_access_and_abort_order.py`. W-I5 on `check_readable` is in `test_tools.py`.

**Negative controls** (`data/13-wp131-python-implementation/mutants-r6.log`; `mutants.log` kept as the remediation-1 record): **32/32 detected**.
- The two pre-G1 C012 mutants targeted the superseded `file_info` access step; they are replaced by revision 5/6's table:
  - `site_from_code_in_normal_mode`
  - `always_fallback`
  - `skip_access_stage`
  - `not_supported_as_access_failure`
  - `file_info_before_read`
  - `fallback_skips_checkpoint_249`
  - `no_checkpoint_249`
- New I001 mutants run against the Python witnesses:
  - `settle_point_check_removed` (fails W-I1, W-I2, W-I3; it is exactly the candidate `61f40e4d` behaviour Codex's witness exposed);
  - `result_rechecked_after_settling` (fails W-I4).
- `cancel_in_flight` and `signal_passed_to_fs` stay.

**Disclosed divergences:** W-1..W-3 and F-1 (`spec/tools.md`, "Disclosed divergences of that mapping"). The Layer-12 follow-ups remain separate and unauthorized: `minion-agent#65`, `#66`, `#67`, `#69`, `#70`.

**Gates (fresh, remediated tree; Windows, Python 3.13.5, pinned ICU 78.3):**
- `pytest`: 2046 passed, 11 skipped, 19 xfailed;
- coverage 100.00%;
- `ruff check` clean; `mypy --strict` clean (91 files);
- conformance + manifest suites pass.

Canonical `builtin_tool` scenarios: 45.

**Linux** (`python:3.12-slim`, uid 1000, without PyICU):
- all `read` witnesses pass, including the real POSIX mode-000 file/directory (W-G6/W-G12) and "is a directory" (W-G11, direct and through a symlink);
- the 22 `read` canonical scenarios pass;
