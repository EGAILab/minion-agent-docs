...
**Trigger:** the same Rust Layer-03 implementation evidence (PR #4,
`2519fc1565ff40ffeb8aa047bc2d3f0aa8bef512`, Rust head `31ed6698a1e4a9f5d3134d2c2b1788f920ceb330`)
that reopened `LLM-F011` also surfaced five Session-owned findings, each independently reproduced
against Rust's real merged source, pinned Pi, and Python's current implementation rather than
mechanically accepted from the review. Per the governance guardrail this pass adds
(`process/implementation-conformance-workflow.md` §4.6), later-language implementation evidence that
reveals a shared-contract ambiguity or a certified-implementation violation must trigger a delta
audit of the owning layer before being classified as merely local hardening.

**Scope discipline:** reopens only `SES-F004`..`SES-F008`'s exact semantics. Does not restart Layer
