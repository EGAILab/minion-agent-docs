# Layer 12 WP-12.1 -- `L12-PY-R002` root characterization, revision 5
...
result but withheld agreement on three narrow assurance defects. Revision 3
(`minion-agent-docs#121` @ `e433fce68d192f86d4514110cb9762377d09682a`) fixed all three.

Revision 3 was independently re-reviewed again (`minion-agent-docs#120` @
`93a44820695d4ec6ce413230cfa0dac1b31d2663`), which independently reconfirmed the exact witnesses,
all 8,246 keys across every dataset, a fresh isolated `ada-url==1.15.3` run, the Ada LTR-bidi
source bug and its Node controls, and the 55/61 taxonomy, and withheld agreement on two further
defects (`compare_oracles.py`'s console crash and missing integrity checks; the backwards
"`ada-url==1.15.3` postdates the fix" claim). Revision 4 (`minion-agent-docs#121` @
`28eda819295355b97e8e5072b02af5fdcc87f347`) fixed both.

Revision 4 was independently re-reviewed a third time (`minion-agent-docs#120` @
`e38eb25d2b1e6872e368a0dd6c99669da7f9411b`). That review **independently reconfirmed Windows/
cp1252 execution, the `idna` version guard, the raw-dataset duplicate/key-set checks, explicit
validation of all 55 Group-A cases, and Strategy A's own substantive evidence** -- none of that
is revisited in this revision. It withheld agreement on two further, narrower, purely mechanical
defects:

1. `compare_oracles.py`'s own `check_corpus_integrity` cross-checked the five oracle-OUTPUT
   datasets against each other and against a freshly in-memory-regenerated corpus, but never
   loaded and cross-checked the COMMITTED `systematic_corpus.txt` FILE ITSELF -- the review
   demonstrated this concretely by removing the `xn--3pc` witness line from the committed file
   and observing the script still exit `0`, since none of the five already-generated
   oracle-output `.txt` files were affected by that edit.
2. One active sentence in this artifact (section 6, not section 8 where the correction was
   already applied) still read `` `ada-url==1.15.3` has neither defect `` -- an internal
   inconsistency with section 8's own already-corrected finding that it reproduces Group A's bug.

This revision corrects both. `check_corpus_integrity` now loads `systematic_corpus.txt` directly
and includes it in the cross-dataset key-set check (re-verified: removing `xn--3pc` from the
committed file now makes the script exit `1` with an explicit `INTEGRITY FAILURES` report,
restoring it returns to exit `0`). The residual sentence is corrected to match section 8. Neither
fix touches the underlying Strategy A/taxonomy conclusions, which the third re-review already
independently reconfirmed. `minion-agent#44` remains UNCHANGED since
`1848873fc9626b699990a29b1f35c7baba78cc34`. `minion-agent-rust/**` is untouched. `L12-PY-R004` is
not reopened.
...
recommendation (section 13): `ada-url==1.15.3` avoids Group B (section 7), and -- per section 8's
own correction -- REPRODUCES Group A rather than avoiding it, which is REQUIRED for, not in
tension with, its exact parity with the pinned oracle.
...
20. ada-url==1.15.3's Group-A claim is correct
    and positively evidenced.................. CONFIRMED (check_ada_url_1153_reproduces_group_a_bug;
                                                  ...
                                                  "predates the fix" throughout, including the
                                                  one residual section-6 sentence a second
                                                  re-review found)
21. check_corpus_integrity validates the
    COMMITTED systematic_corpus.txt file, not
    only the oracle-output datasets............ CONFIRMED (load_corpus_file reads the file
                                                  directly; re-verified by removing xn--3pc from
                                                  the committed file -- now fails loudly with
                                                  exit code 1 and an explicit INTEGRITY FAILURES
                                                  report, where it previously exited 0)
```
...
ACCEPTANCE WITNESSES
    ...
    assurance/layers/data/12-python-r002-ada-oracle/ (oracle.cpp, vendored Ada 2.9.2 source
        ...
        oracle outputs, compare_oracles.py [with corpus-integrity checks -- now including
        the committed systematic_corpus.txt file itself -- and UTF-8-safe output], README.md)
