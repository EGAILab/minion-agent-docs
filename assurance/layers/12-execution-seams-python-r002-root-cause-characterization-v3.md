# Layer 12 WP-12.1 -- `L12-PY-R002` root characterization, revision 3
...
`8c1469c1de13cf0a70dab9f6d4bcfd8abcd8e32e`). The review **independently reproduced the central
Strategy A result** -- `ada-url==1.15.3` matches the direct Ada 2.9.2 oracle on all 8,244 rows of
the committed systematic corpus -- and confirmed the source-chain citations, all reported
comparison counts, and a fresh isolated `ada-url==1.15.3` run of its own. It withheld agreement on
three narrow assurance defects, none of which touch the Strategy A conclusion itself:

1. The exact `xn--3pc`/`xn--8g0n` witnesses were spot-checked in prose but were not literally
   present in the committed permanent differential corpus (`systematic_corpus.txt` only
   contained the leading-`"a"`-prefixed PROXY shapes of those two codepoints).
2. The committed `compare_oracles.py` referenced filenames that did not match what was actually
   committed, omitted the decisive `ada-url==1.15.3` comparison from the script itself (that
   number came from an uncommitted one-off check), and the claimed "deterministic generator" for
   `systematic_corpus.txt` did not exist as a committed, runnable file.
3. The claim that "all [115] mismatches" between the direct oracle and `ada-url==4.0.0` are
   Unicode-codepoint-assignment-boundary differences was contradicted by the artifact's own cited
   example, `U+05D0 HEBREW LETTER ALEF` -- a codepoint assigned since Unicode 1.0, not a
   recently-assigned one.

This revision corrects all three. `minion-agent#44` remains UNCHANGED since
`1848873fc9626b699990a29b1f35c7baba78cc34`. `minion-agent-rust/**` is untouched. `L12-PY-R004`
is not reopened.
...
ACCEPTANCE WITNESSES
    ...
    assurance/layers/data/12-python-r002-ada-oracle/ (oracle.cpp, vendored Ada 2.9.2 source
        with checksums, generate_corpus.py, systematic_corpus.txt [8246 cases, including
        xn--3pc/xn--8g0n verbatim], all raw oracle outputs, compare_oracles.py, README.md)
